...
export default function BlockDetails({ block, diagnostic, tasks, territory, identity, onUpdateBlock }) {
  ...
  async function handleStateTransition(targetStatus) {
    if (!block || !identity || transitioning) return;
    setTransitioning(true);
    setTransitionError(null);
    try {
      const updated = await transitionBlock(
        identity.plan_id,
        block.block_id,
        targetStatus,
        "block_details_ui"
  ...
  const currentStatus = block.status || "DRAFT";
  ...
  return (
    <section className="block-details" aria-labelledby="block-details-heading">
      <div className="workspace-column-heading" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          <span className="planner-kicker" style={{ fontFamily: "var(--font-mono)" }}>
            POSSESSION #{block.block_id}
          </span>
          <h2 id="block-details-heading">Block explanation</h2>
        </div>
        <span className={`block-status-badge status-${currentStatus.toLowerCase()}`}>
          {statusBadgeText}
        </span>
      </div>
      ...
      <div className="block-details-content">
        ...
        <div className="block-details-location">
          ...
          <span>
            Capacity: <code style={{ fontFamily: "var(--font-mono)" }}>{(block.capacity_resource_ids?.length ? block.capacity_resource_ids : protectedSections).join(" + ")}</code>
            {block.track_ids?.length ? ` · Track ${block.track_ids.join(" + ")}` : " · Conservative whole-section protection"}
          </span>
        </div>

        {/* Operational Status Actions */}
        {identity && (
          <div className="block-control-actions" style={{ margin: "10px 0", padding: "10px", background: "var(--paper)", border: "1px solid var(--line)", borderRadius: "2px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "6px" }}>
              <small style={{ fontFamily: "var(--font-mono)", fontSize: "10px", textTransform: "uppercase", color: "var(--muted)", fontWeight: 700 }}>
                Operations Lifecycle Control
              </small>
              {transitioning && <small style={{ color: "var(--train)", fontFamily: "var(--font-mono)" }}>Transitioning…</small>}
            </div>
            <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
              {currentStatus === "DRAFT" && (
                <button
                  ...
                  onClick={() => handleStateTransition("FROZEN")}
                  title="Freeze this block to preserve its schedule window against automatic perturbation"
                >
                  ❄ Freeze Window
                </button>
              )}
              {currentStatus === "FROZEN" && (
                <>
                  <button
                    ...
                    title="Mark possession as physically underway on the track"
                  >
                  ...
                  <button
                    ...
                    title="Unfreeze back to flexible draft"
                  >
                    ↩ Unfreeze
                  </button>
                </>
              ...
              {currentStatus === "IN_PROGRESS" && (
                <button
                  ...
                  title="Certify track cleared and release block"
                >
        ...
        <div className="block-details-dept-pill">
          ...
          <small>{block.integrated ? "Integrated multi-department possession" : "Individual possession"}</small>
        </div>
