  ...
  return <div className="operations-shell">
    ...
    <section className="operations-card">
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "16px", marginBottom: "8px" }}>
        <h3 style={{ margin: 0 }}>RailSync Copilot</h3>
        <span className="noc-badge-pill" style={{ color: "var(--confirm)", borderColor: "rgba(16, 185, 129, 0.4)", background: "rgba(16, 185, 129, 0.12)" }}>
          SOLVER-GROUNDED · ZERO HALLUCINATION
        </span>
      </div>
      <p style={{ fontSize: "11px", color: "var(--muted)", margin: "0 0 10px" }}>
        Directly queries the OR-Tools CP-SAT math model. Every answer is mathematically bounded and constraint-verified.
      </p>
      <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginBottom: "8px" }}>
        <button
          ...
          onClick={() => setCopilotQuestion("Why was this possession scheduled in this window?")}
        >
          Why this window?
        </button>
        <button
          ...
          className="noc-theme-toggle"
          style={{ fontSize: "10px", padding: "3px 8px" }}
          onClick={() => setCopilotQuestion("What if maintenance duration extends +15 minutes?")}
        >
          What if +15 min?
        </button>
        <button
          ...
          className="noc-theme-toggle"
          style={{ fontSize: "10px", padding: "3px 8px" }}
          onClick={() => setCopilotQuestion("Can we co-locate S&T signal work with TRD power block?")}
        >
          Co-locate S&T + TRD?
        </button>
      </div>
      <form className="copilot-form" onSubmit={ask}>
        <input aria-label="Ask RailSync Copilot" value={copilotQuestion} onChange={(event) => setCopilotQuestion(event.target.value)} placeholder="Ask Copilot or test a what-if perturbation..." />
        <button type="submit" disabled={busy}>Ask</button>
      </form>
      {copilotAnswer ? (
        <div className="copilot-answer">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "4px" }}>
            <strong style={{ fontFamily: "var(--font-mono)", fontSize: "11px", color: "var(--confirm)" }}>
              {copilotAnswer.engine.replaceAll("_", " ")}
            </strong>
          ...
          {copilotAnswer.action_preview ? <small>Action routed through CP-SAT; preview is not applied to active plan.</small> : null}
        </div>
      ) : null}
